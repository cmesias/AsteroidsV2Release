...
double fRandA(double fMin, double fMax)
{
...
void Asteroid::loadResources(SDL_Renderer* gRenderer){
	gAsteroid.loadFromFile("img/asteroid.png", gRenderer);
...
void Asteroid::initAsteroid(Asteroid asteroid[]) {
	for (int i = 0; i < 128; i++) {
		asteroid[i].x 				= 0;
		asteroid[i].y 				= 0;
		asteroid[i].w 				= 0;
...
void Asteroid::spawnAsteroidAngle(Asteroid asteroid[],
	...
	for (int i = 0; i < 128; i++)
	{
		if (!asteroid[i].alive)
		{
			...
			asteroid[i].damagedTimer 	= 0;
			asteroid[i].health			= 100;
			asteroid[i].angleSpe 		= fRandA(1.0, 2.5);
...
void Asteroid::spawnAsteroid(Asteroid asteroid[], int spawnx, int spawny, int w, int h, int targetx,int targety, float speed, string type) {
	for (int i = 0; i < 128; i++) {
		if (!asteroid[i].alive) {
			asteroid[i].health 			= 100;
...
void Asteroid::updateAsteroid(Asteroid asteroid[],
							  ...
							  int visionX, int visionY, int objectRadius,
	...
	for (int i = 0; i < 128; i++)
	{
		if (asteroid[i].alive)
		{
			...
			if (asteroid[i].health <= 0)
			{
				...
				g_dummy0.spawn(explosion, asteroid[i].x+asteroid[i].w/2-asteroid[i].w/2, asteroid[i].y+asteroid[i].w/2-asteroid[i].w/2,
										  asteroid[i].w, asteroid[i].w, 255, "normal", sAtariBoom);
				...
				if (asteroid[i].type=="large"){
					for (int j = 0; j < 2; j++) {
			            double randAngle 	= fRandA(0.0, 360.0);
